        ...
        return (
            <div className="bg-slate-50 min-h-screen pt-48 pb-24 px-6 text-center flex items-center justify-center">
                ...
                >
                    ...
                    <h1 className="text-4xl font-black uppercase tracking-tighter mb-4 text-slate-900">Enquiry Captured</h1>
                    ...
                    <button
                        ...
                        className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-medical-700 transition-all shadow-lg"
                    >
                        New Enquiry Request
                    </button>
                </motion.div>
            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
                    ...
                    <div className="lg:sticky lg:top-32">
                        ...
                        <h2 className="text-5xl font-black tracking-tighter text-slate-900 uppercase leading-[0.9] mb-8">
                            Bulk Enquiry <br />
                            <span className="normal-case text-medical-600 italic font-medium">& Extraction</span>
                        </h2>
                        ...
                        <div className="space-y-6 mb-12">
                            {[
                                ...
                                <div key={item.label} className="flex items-center gap-6 group">
                                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-medical-600 border border-slate-100 shadow-sm transition-all duration-300">
                                        <item.icon className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.label}</p>
                                        {item.link ? (
                                            <a href={item.link} className="text-sm font-bold text-slate-900 hover:text-medical-600 transition-colors">{item.val}</a>
                                        ) : (
                        ...
                        <div className="bg-medical-50 p-8 rounded-[2.5rem] border border-medical-100 flex items-center gap-6">
                            <div className="w-16 h-16 bg-[#25D366] rounded-3xl flex items-center justify-center text-white shadow-lg shadow-green-500/20">
                                <ChatBubbleLeftRightIcon className="w-8 h-8" />
                            </div>
                            <div>
                                <h4 className="text-sm font-black text-green-600 uppercase tracking-widest mb-1">WhatsApp Fast-Track</h4>
                                            ...
                                            <div className="space-y-6">
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full bg-slate-50 border-0 rounded-2xl p-5 text-sm font-bold focus:ring-2 focus:ring-medical-500 outline-none"
                                                    />
                                                </div>
                                                <div>
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                    {SPECIALTIES.map((spec) => (
                                                        <label key={spec.id} className={`group cursor-pointer p-6 rounded-3xl border-2 transition-all duration-300 relative overflow-hidden ${formData.specialty === spec.id ? 'border-medical-600 bg-medical-50/50' : 'border-slate-100 hover:border-slate-200 bg-slate-50'
                                                            }`}>
                                                            <input type="radio" {...register("specialty")} value={spec.id} className="hidden" />
                                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 transition-colors ${formData.specialty === spec.id ? 'bg-medical-700 text-white' : 'bg-white text-slate-300'
                                                                }`}>
                                                                <spec.icon className="w-5 h-5" />
                                                            </div>
                                                            <h4 className="text-xs font-black text-slate-900 mb-1 uppercase tracking-tight">{spec.name}</h4>
                                                            <p className="text-[10px] text-slate-400 font-medium italic">{spec.desc}</p>
                                                            {formData.specialty === spec.id && (
                                                                <motion.div layoutId="glow" className="absolute top-0 right-0 w-24 h-24 bg-medical-600/10 blur-[40px] rounded-full"></motion.div>
                                                            )}
                                                        </label>
                                                    ))}
                                                </div>
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    {['Single Batch', 'Monthly Contract', 'Annual Tender', 'Spot Purchase'].map(v => (
                                                        <label key={v} className={`p-4 rounded-xl border-2 text-center cursor-pointer transition-all ${formData.volume === v ? 'border-medical-700 bg-medical-50 text-medical-700' : 'border-slate-50 bg-slate-50 text-slate-400'
                                                            }`}>
                                            ...
                                            <div className="flex gap-4">
                                                ...
                                                <button
                                                    ...
                                                    className="flex-1 bg-medical-700 text-white font-black text-xs uppercase tracking-[0.2em] py-5 rounded-2xl hover:bg-medical-900 shadow-xl shadow-medical-900/20 transition-all flex items-center justify-center gap-3"
                                                >
                                                    {isSubmitting ? (
